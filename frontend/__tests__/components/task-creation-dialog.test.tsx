...
describe('TaskCreationDialog', () => {
  ...
  describe('Due Date Functionality', () => {
    ...
    it('includes due date in task creation request when provided', async () => {
      ...
      // Calculate expected UTC time (timezone-agnostic)
      const expectedUTCTime = new Date(dueDateTime).toISOString();
      ...
      await waitFor(() => {
        expect(projectService.createTask).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            dueDateTime: expectedUTCTime, // Dynamic UTC conversion based on current timezone
          })
        );
      });
    });
