...
// Helper function to get expected values based on current timezone
const getExpectedDateValues = (utcDateString: string) => {
  ...
  return {
    ...
    formattedPreview: `Due: ${date.toLocaleString('en-SG', {
      dateStyle: 'medium',
      timeStyle: 'short'
    })}`
  };
...
describe('TaskUpdateDialog - Due Date Features', () => {
  ...
  test('allows changing the due date', async () => {
    ...
    const expectedValues = getExpectedDateValues(mockTask.dueDateTime!);
    render(<TaskUpdateDialog {...defaultProps} />);
  ...
  test('shows due date preview when date is set', () => {
    ...
    const previewTexts = screen.getAllByText((content, element) => {
      return element?.textContent === expectedValues.formattedPreview;
    });
